...
def unload_whisper():
    """Libera Whisper de VRAM para dejar espacio al TTS."""
    global _whisper
    if _whisper is not None:
        ...
        if DEVICE == "cuda":
            ...
            print(f"  [GPU] Whisper descargado. VRAM: {free/1024**3:.1f} GB libre / {total/1024**3:.1f} GB total")
...
def synthesize_translated(
    ...
    # Limpiar VRAM antes de TTS para evitar fragmentacion y degradacion
    if DEVICE == "cuda":
        torch.cuda.empty_cache()
        free, total = torch.cuda.mem_get_info()
        print(f"  [GPU] VRAM pre-TTS: {free/1024**3:.1f} GB libre / {total/1024**3:.1f} GB total")
    ...
    # -- Agrupacion adaptativa segun duracion del audio --
    # Chatterbox sweet spot: ~180-230 chars/grupo (autoregresivo, textos largos = lento)
    # Audio corto: agrupacion agresiva pero respetando el sweet spot de chars
    # Audio medio/largo: agrupacion mas granular para naturalidad
    if original_duration < 45:
        groups = _group_segments_for_tts(segments, translated_texts, max_chars=200, max_gap=5.0, min_chars=140)
        PACING_FACTOR = 0.20
        mode_label = "corto"
    elif original_duration < 120:
        groups = _group_segments_for_tts(segments, translated_texts, max_chars=220, max_gap=3.0, min_chars=100)
        PACING_FACTOR = 0.30
        mode_label = "medio"
    elif original_duration < 180:
        groups = _group_segments_for_tts(segments, translated_texts, max_chars=230, max_gap=2.5)
        PACING_FACTOR = 0.40
        mode_label = "semi-largo"
    else:
        groups = _group_segments_for_tts(segments, translated_texts, max_chars=_TTS_MAX_CHARS)
        PACING_FACTOR = 0.45
        mode_label = "largo"
    tts_chunk_limit = _TTS_MAX_CHARS
    total = len(groups)
    print(f"  [TTS] {len(segments)} segmentos -> {total} grupos TTS "
          f"(modo {mode_label}, pacing={PACING_FACTOR}) ({provider.name})")
    ...
    try:
        for gi, group in enumerate(groups):
            ...
            # 1. Silencio original antes de este grupo
            ...
            text_chunks = _split_long_text(translated, max_chars=tts_chunk_limit)
            n_segs_in_group = len(group["seg_indices"])
            if len(text_chunks) > 1:
                print(f"  [TTS {done}/{total}] {group['start']:.1f}s-{group['end']:.1f}s "
                      f"({elapsed:.0f}s) {n_segs_in_group} segs | {translated[:70]!r} [{len(text_chunks)} fragmentos]")
            else:
                print(f"  [TTS {done}/{total}] {group['start']:.1f}s-{group['end']:.1f}s "
                      f"({elapsed:.0f}s) {n_segs_in_group} segs | {translated[:70]!r}")
            ...
            orig_group_dur = group["end"] - group["start"]
            time_saved = max(0.0, orig_group_dur - tts_dur)
...
def pipeline(
    ...
    try:
        ...
        # Liberar Whisper de VRAM antes de TTS
        unload_whisper()
...
def _group_segments_for_tts(
    ...
    - Pausa >=1.5s: pausa larga, corta grupo (salvo si min_chars no se alcanza)
    ...
    min_chars: si el grupo actual tiene menos de esto, fuerza fusion con el siguiente
               (util para audio corto donde queremos menos llamadas TTS).
    ...
    for idx, (seg, text) in enumerate(zip(segments, tts_texts)):
        ...
        if current is None:
            ...
            # Si el grupo actual es demasiado pequeno, forzar fusion
            if min_chars > 0 and current_len < min_chars and combined_len <= max_chars + 20:
                current["text"] += " " + text
                current["end"] = seg["end"]
            ...
            elif ends_sentence and current_len >= 150:
    ...
    with gr.Row():
        with gr.Column(scale=1):
            ...
            with gr.Accordion("Flujo del pipeline", open=False):
                ...
                gr.Markdown(
                    "| Paso | Que hace | 30s audio | 5 min audio | Cuello de botella |\n"
                    "|------|----------|-----------|-------------|-------------------|\n"
                    "| **1. Whisper** | Transcripcion con timestamps | ~5s | ~20s | Bajo |\n"
                    "| **1b. Descarga** | Libera Whisper de VRAM (~3.7 GB) | <1s | <1s | Ninguno |\n"
                    "| **2. Gemini** | Traduccion (1 llamada API) | ~2s | ~3-5s | Minimo |\n"
                    "| **3. TTS** | Sintesis voz por grupo (~200 chars) | ~16s | ~3-4 min | **PRINCIPAL** ~0.7x |\n"
                    "| **4. Mezcla** | Pausas + escritura WAV/MP3 | <1s | <1s | Ninguno |\n\n"
                    "**Rendimiento actual:** ~0.7x tiempo real (genera mas rapido que la duracion del audio).\n\n"
                    "**Notas:**\n"
                    "- Whisper se **descarga de VRAM** despues de transcribir para liberar ~3.7 GB para TTS.\n"
                    "- Los segmentos se **agrupan adaptativamente** segun duracion (~200 chars/grupo = sweet spot).\n"
