...
- **Dos pipelines** optimizados:
  - Audio corto (<3 min): agrupacion adaptativa por duracion
  - Audio largo (>3 min): Whisper completo + traduccion por lotes + TTS agrupado con resume
- **Gestion inteligente de VRAM** — Descarga Whisper antes de TTS para liberar ~3.7 GB
...
```
Audio → Whisper (transcripcion) → [Whisper descargado de VRAM] → Gemini Flash (traduccion) → Chatterbox TTS (agrupado) → MP3
```

### Pipeline largo (>3 min)

```
Audio → Whisper COMPLETO → [Whisper descargado de VRAM] → Traduccion en lotes (con contexto) → TTS agrupado → Resume/Checkpoint → MP3
```
...
#### 3. Agrupacion adaptativa de segmentos para TTS
- **Problema:** Demasiadas llamadas TTS = overhead CUDA por cada una. Pero un solo grupo gigante es peor (Chatterbox es autoregresivo: textos largos = exponencialmente mas lento)
- **Sweet spot:** ~180-230 chars por grupo (optimo para Chatterbox en GPUs de 8GB)
- **Solucion:** Agrupacion adaptativa segun duracion del audio:

  | Duracion | max_chars | min_chars | max_gap | Pacing |
  |---|---|---|---|---|
  | < 45s | 200 | 140 | 5.0s | 0.20 |
  | 45-120s | 220 | 100 | 3.0s | 0.30 |
  | 120-180s | 230 | 0 | 2.5s | 0.40 |
  | > 180s | 150 | 0 | 2.0s | 0.45 |

- `min_chars`: fuerza fusion de grupos pequenos para reducir llamadas TTS
- `max_gap`: permite agrupar segmentos con pausas mas largas en audio corto
- **Impacto:** Audio de 28s pasa de 4 llamadas TTS → 2 llamadas, de 3.8x → 0.7x tiempo real
...
#### 9. Gestion inteligente de VRAM
- **Problema:** Whisper + Chatterbox TTS juntos saturan los 8GB de VRAM (0.0 GB libre), causando rendimiento degradado e inconsistente (2x-5x en vez de 0.7x)
- **Solucion:** `unload_whisper()` descarga Whisper de VRAM despues de transcribir, antes de iniciar TTS. Se llama `torch.cuda.empty_cache()` para eliminar fragmentacion
- **Impacto:** VRAM libre pasa de 0.0 GB → 3.7 GB. Rendimiento TTS consistente ~0.7x tiempo real
- **Costo:** Whisper se recarga (~5-10s) en la siguiente traduccion (lazy-load automatico)

#### 10. Optimizaciones de GPU
...
#### 11. Pacing y ritmo
- Factor de pacing adaptativo: 0.20 (corto) → 0.45 (largo)
...
|---|---|---|---|---|
| 28s | 4 | 2 | ~16s TTS | **0.7x** |
| 4m 55s | 46 | ~22 | ~4-5 min | **0.7-1.2x** |

> **Nota:** El rendimiento depende de la VRAM disponible. La app descarga Whisper
> automaticamente antes de TTS para maximizar VRAM libre (~3.7 GB en GPUs de 8GB).
> Si el rendimiento baja (>2x), verifica con `nvidia-smi` que no haya procesos
> consumiendo VRAM. La app incluye logs de VRAM y timing por grupo TTS.
